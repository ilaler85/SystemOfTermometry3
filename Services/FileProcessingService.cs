...
namespace SystemOfTermometry2.Services
{
    ...
    {
        private static string settingsFolder = Path.Combine(Application.CommonAppDataPath, ".folder");
        private static string pswdFolder = Path.Combine(Application.CommonAppDataPath, ".pswd");

        public static string ExportDir
        {
            get => Path.Combine(Application.CommonAppDataPath, "Export");
        }
        public static string ExportOverheatDir
        {
            get => Path.Combine(Application.CommonAppDataPath, "ExportOverheat");
        }

        public static string LogDir
        {
            get => Path.Combine(Application.CommonAppDataPath, "Log");
        }

        public static string WorkingDir
        {
            get => Application.CommonAppDataPath;
        }
        ...
        static public byte[][] getHashPassword(string file)
        {
            ...
            try
            {
                ...
                //stream.ReadBlock(tmp, 0, 20);
                //pswdSalt[0] = Encoding.ASCII.GetBytes(tmp);
                //stream.ReadBlock(tmp2, 0, 20);
                //pswdSalt[1] = Encoding.ASCII.GetBytes(tmp2);

                //pswdSalt[0] = Encoding.ASCII.GetBytes(stream.ReadLine());
                //pswdSalt[1] = Encoding.ASCII.GetBytes(stream.ReadLine());
                stream.Close();
                return pswdSalt;
            }
            catch (Exception ex)
